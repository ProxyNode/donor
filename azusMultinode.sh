...
COIN3l=azus
COINDAEMON=azusd
COINDAEMONCLI=azus-cli
COINCORE=.azus
COINCONFIG=azus.conf
COINHOME=/home/azus
#wallet downnload and extractions commands
DOWNLOADCOINFILES=https://github.com/Remapper/azus/releases/download/1.0.0.0/azus.zip
COINFILES=azus.zip
#need DECOMPRESS
DECOMPRESS='unzip'
#rocketstrap
NEBootStrap=xyz #http://nullentry.com/chain/PRX/rocketstrap.rar
AddNodeList=x #http://nullentry.com/chain/PRX/addnodes.tbl
ADDNODE0=x #66.42.113.222:12195
ADDNODE1=x #107.191.50.119:12195
ADDNODE2=x #51.15.118.135:12195
ADDNODE3=x #159.89.9.125:12195
ADDNODE4=x #178.128.61.190:12195
ADDNODE5=x # 35.181.52.124:12195
ADDNODE6=x #209.97.166.255:12195
ADDNODE7=x #13.58.228.147:12195
ADDNODE8=x #221.152.128.194:12195
ADDNODE9=x #88.85.160.29:12195
COINPORT=26210
COINRPCPORT=32101
...
#warppoint1
  ...
  #Test_Pause
  bash <(curl -Ls https://raw.githubusercontent.com/ProxyNode/donor/master/welcome.sh)
...
#warp2
  ...
  if [ -d /home/${COINl}${nodeunit} ]; then
    ...
    ${COIN}-cli -datadir=${COINHOME}${nodeunit}/${COINCORE} stop
    sleep 7
    echo -e ${CLEAR}
...
#warppoint1
  ...
  if [ -d /home/${COINl}${nodeunit} ]; then
    if [ -z ${nodeunit} ]; then
    echo -e ${GREEN}"Found ${COIN1}-Oldnode Installation Found - /home/${COINl}" ${CLEAR}
  else
    echo -e ${GREEN}"Found ${COIN}-${nodeunit} Installation Found - /home/${COINl}${nodeunit}" ${CLEAR}
  fi
  ${COINDAEMONCLI} -datadir=${COINHOME}${nodeunit}/${COINCORE} masternode status &> ${DPATH}${COIN1}MN${nodeunit}.tmp
    if grep -q "Hot node, waiting for remote activation" ${DPATH}${COIN1}MN${nodeunit}.tmp; then
      echo -e ${YELLOW} "Masternode Ready, waiting for activation from Wallet" ${CLEAR}
    elif grep -q "Loading block index..." ${DPATH}${COIN1}MN${nodeunit}.tmp; then
    echo -e ${YELLOW} "Masternode is still loading block Index, please wait." ${CLEAR}
    elif grep -q "Masternode successfully started" ${DPATH}${COIN1}MN${nodeunit}.tmp; then
      echo -e ${GREEN} "Masternode Successfully Started" ${CLEAR}
    elif grep -q "Masternode not found in the list of available masternodes. Current status: Node just started, not yet activated" ${DPATH}${COIN1}MN${nodeunit}.tmp; then
      echo -e ${YELLOW} "Masternode is loading blocks, Please Wait " ${CLEAR}
    elif grep -q "error: couldn't connect to server" ${DPATH}${COIN1}MN${nodeunit}.tmp; then
  ...
  Function_AptGet_Update() {
    echo -e ${RED}"Update Apps? Y/N"${CLEAR}
    read yesno
    YIP="Y"
    if [ $yesno == $YIP ]; then
...
#        fi
  ...
  sudo apt-get -y install libdb4.8++-dev
  echo -e ${YELLOW} "This can take a while"${CLEAR}
...
#        echo -e "Testing Version - Not for public release."
  ...
  sudo chmod 755 -R  /usr/local/bin/${COIN31}*
  #rm -rf /root/${COIN3l}
  }
...
Function_Glances(){
  ...
  if [ ! -f /home/${COINl}${nodeunit}/.${COINl}/${COINCONFIG} ]; then
  echo -e "${GREEN}Halting and Replicating First ${COIN} Node${CLEAR}"
  echo
  sleep 2
  Function_Build_Node_Directories
  cd /
...
#  echo ${Install_Count}
  ...
  function_Donations(){
  #attempt to move legacy masternode
    bash <(curl -Ls https://raw.githubusercontent.com/ProxyNode/donor/master/donations.sh)
    pause
  }
Function_IP_Table_Check(){
